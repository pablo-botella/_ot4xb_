...
static void rgx_utility( XppParamList pl , DWORD np ) // "operation.flags:expression" , string , replace  // test,replace,exec
{
   ...
   if( conr ){ _conReturn(pl,conr);_conRelease(conr); conr = 0; } else { _ret(pl); }
...
//----------------------------------------------------------------------------------------------------------------------
static void create_class_rgx_map_table( XppParamList pl)
{
   ContainerHandle conco = _conClsObj("_RGX");
   if( conco == NULLCONTAINER )
   {
...
{
   ContainerHandle conco = _conClsObj("_STRING_MAP_TABLE");
   if( conco == NULLCONTAINER )
   {
      TXbClass * pc = new TXbClass;pc->ClassName("_STRING_MAP_TABLE");
      // -----
      pc->EXPORTED();
      // -----
      pc->Var("m_handle");
      // -----
      pc->MethodCB("init"       ,"{|s,search,replace| s:m_handle := _STRING_MAP_TABLE_NEW(@s:m_handle,search,replace) , s }" );
      pc->MethodCB("Destroy"    ,"{|s|s:m_handle := _STRING_MAP_TABLE_DESTROY(@s:m_handle ) , s ");
      pc->MethodCB("Exec"       ,"{|s,cc| _STRING_MAP_TABLE_EXEC(@s:m_handle , __vstr(cc,'')) }");
      // -----
      conco = pc->Create();
      delete pc;
      if( conco == NULLCONTAINER )
...
// ---------------------------------------------------------------------------------------------------------------------
_XPP_REG_FUN_( _STRING_MAP_TABLE_NEW     )  // _STRING_MAP_TABLE_NEW(@handle,search,replace)
{
   TXppParamList xpp(pl,4);
   TStringMapTable * smp = reinterpret_cast<TStringMapTable *>( xpp[1]->GetDWord() );
   if( smp ){ delete smp; }
   smp = 0;
   if( xpp[2]->CheckType( XPP_CHARACTER ) &&  xpp[3]->CheckType( XPP_CHARACTER ) )
   {
      smp = new TStringMapTable( xpp[2]->LockStr() , xpp[3]->LockStr() )
   }
   xpp[1]->PutDWord( (DWORD) smp );
   xpp[0]->PutDWord( (DWORD) smp );
}
// ---------------------------------------------------------------------------------------------------------------------
_XPP_REG_FUN_( _STRING_MAP_TABLE_DESTROY )  // _STRING_MAP_TABLE_DESTROY(@handle)
{
   TXppParamList xpp(pl,1);
   TStringMapTable * smp = reinterpret_cast<TStringMapTable *>( xpp[1]->GetDWord() );
   if( smp ){ delete smp; }
...
// ---------------------------------------------------------------------------------------------------------------------
_XPP_REG_FUN_( _STRING_MAP_TABLE_EXEC    )  // _STRING_MAP_TABLE_EXEC(@handle,string)
{
   TXppParamList xpp(pl,2);
   TStringMapTable * smp = reinterpret_cast<TStringMapTable *>( xpp[1]->GetDWord() );
   if( smp  && xpp[2]->CheckType( XPP_CHARACTER ) )
   {
       LPSTR p = smp->exec( xpp[2]->LockStr() ); 
       if( p )
       {
          xpp[0]->PutStr( p );
          _xfree( (void*) p );
...
// ---------------------------------------------------------------------------------------------------------------------
TStringMapTable::TStringMapTable(LPSTR search , LPSTR replace )
{
   ...
   m_ht =  new THashTable();
   m_ht->m_pfnOnDestroyItem = THDictItem::_OnDestroyItem_;
   
   IRegExp2* hrgx = _rgx_init_interface(0);
   m_rgx_interface = (void*) hrgx;
   
   if( search && replace )
   {
      TList search_list(
      DWORD cb = _xstrlen(search);
...
// ---------------------------------------------------------------------------------------------------------------------
TStringMapTable::~TStringMapTable()
{
   ;
}
// ---------------------------------------------------------------------------------------------------------------------
LPSTR TStringMapTable::exec( LPSTR cc )
{
   return cc;
}
// ---------------------------------------------------------------------------------------------------------------------

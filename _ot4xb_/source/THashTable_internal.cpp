...
#include <string.h>
// -----------------------------------------------------------------------------------------------------------------
THashTable::THashTable(DWORD nBuckets,DWORD dwMaxPopulation)
{
   m_cs_cargo = 0;
...
// -----------------------------------------------------------------------------------------------------------------
void THashTable::DeleteAllItems( void )
{
  THashItem* pItem = m_pFirstItem;
  while( pItem )
  {
     THashItem* pNext = pItem->m_pNext;
...
// -----------------------------------------------------------------------------------------------------------------
THashItem *  THashBucket::InsertItem( THashTable* pTable , THashItem * pNew) // pOldItem if Replaced
{
   pNew->m_pNext = 0;
   pNew->m_pPrev = 0;
   if( m_pChain )
   {
      THashItem * pItem = m_pChain;
      THashItem * pLast = 0;
      DWORD dw = 0;
      // for( ; ((dw < m_nCount) && (pNew->m_dwCrc > pItem->m_dwCrc)) ; dw++ , pItem = pItem->m_pNext) pLast = pItem;
      for( ; dw < m_nCount; dw++ , pItem = pItem->m_pNext)
      {
         int i = THashTable::Compare( pNew->m_pKey ,pNew->m_cbKey ,pNew->m_dwCrc,
...
// -----------------------------------------------------------------------------------------------------------------
THashItem * THashBucket::_replace_item_(  THashTable* , THashItem * pItem , THashItem * pNew )
{
...
// -----------------------------------------------------------------------------------------------------------------
THashItem * THashBucket::_insert_before_( THashTable* pTable , THashItem * pItem , THashItem * pNew )
{
   pTable->m_nCount++; m_nCount++;
...
// -----------------------------------------------------------------------------------------------------------------
THashItem * THashBucket::_insert_after_(  THashTable* pTable , THashItem * pItem , THashItem * pNew )
{
   pTable->m_nCount++; m_nCount++;
   if( pTable->m_dwPopIndex < m_nCount ) pTable->m_dwPopIndex = m_nCount;
...
// -----------------------------------------------------------------------------------------------------------------
THashItem * THashBucket::_remove_item_(THashTable* pTable , THashItem * pItem)
{
...
// -----------------------------------------------------------------------------------------------------------------
THashItem * THashBucket::FindItem(LPSTR pKey , DWORD cbKey)
{
   if( m_pChain )
   {
      DWORD dwCrc = dwCrc32Lower(0,(LPBYTE) pKey,cbKey);
      THashItem * pItem = m_pChain;
      DWORD dw = 0;
      // for( ; ((dw < m_nCount) && (dwCrc > pItem->m_dwCrc)) ; dw++ , pItem = pItem->m_pNext) ;
      for( dw = 0, pItem = m_pChain; dw < m_nCount; dw++ , pItem = pItem->m_pNext)
      {
         int i = THashTable::Compare(pKey,cbKey,dwCrc,pItem->m_pKey,pItem->m_cbKey ,pItem->m_dwCrc);
         if( i < 0 ) return 0;
         if( i == 0 ) return pItem;
      }
   }
...
// -----------------------------------------------------------------------------------------------------------------
void * THashTable::FindItem(LPSTR pKey , DWORD cbKey)
{
   if( m_nCount )
   {
      if( cbKey == (DWORD) -1 ){ cbKey = _xstrlen(pKey); }      
      DWORD dw = ( _xstrhash(pKey,cbKey)  &  (m_nBuckets -1) );
      THashItem * pItem = m_pBuckets[dw].FindItem(pKey,cbKey);
      if( pItem ) return pItem->m_pData;
   }
   return 0;
}
// -----------------------------------------------------------------------------------------------------------------
THashItem * THashTable::FindItemEx(LPSTR pKey , DWORD cbKey)
{
   if( m_nCount )
   {
      if( cbKey == (DWORD) -1 ){ cbKey = _xstrlen(pKey); }      
      DWORD dw = ( _xstrhash(pKey,cbKey)  &  (m_nBuckets -1) );
      return m_pBuckets[dw].FindItem(pKey,cbKey);
   }
   return 0;
}
// -----------------------------------------------------------------------------------------------------------------
void * THashTable::RemoveItem(LPSTR pKey , DWORD cbKey)
{
   if( m_nCount )
   {
      DWORD dw = ( _xstrhash(pKey,cbKey)  &  (m_nBuckets -1) );
...
// -----------------------------------------------------------------------------------------------------------------
BOOL THashTable::DeleteItem(LPSTR pKey , DWORD cbKey)
{
...
// -----------------------------------------------------------------------------------------------------------------
void * THashTable::InsertItem(LPSTR pKey , DWORD cbKey, void * pData, BOOL bDestroyOnReplace)
{
   if( m_nBuckets < 8 ) ReHash(8);
   // if( (m_dwPopIndex > m_dwMaxPopulation) && ((m_nCount << 1)  > m_nBuckets) ) ReHash(m_nBuckets * 2);
   if( (m_nCount << 1)  > m_nBuckets  ) ReHash(m_nBuckets * 2);
   THashItem * pNew = new THashItem( pKey , cbKey , pData );
   DWORD dw = ( _xstrhash(pKey,cbKey)  &  (m_nBuckets -1) );
   THashItem * pItem = m_pBuckets[dw].InsertItem(this,pNew);
   if( pItem )
   {
...
// -----------------------------------------------------------------------------------------------------------------
THashItem::THashItem(LPSTR pKey , DWORD cbKey , void* pData )
{
      m_pKey  = pKey;
      m_cbKey = cbKey;
      m_pData = pData;
      m_dwCrc = dwCrc32Lower(0,(LPBYTE) pKey,cbKey);
}
// -----------------------------------------------------------------------------------------------------------------
void THashTable::ReHash( DWORD nBuckets)
{
   DWORD dw;
   THashItem* pItem = m_pFirstItem;
   ...
   if( m_pBuckets ) _xfree( (void*) m_pBuckets );
   m_pBuckets     = (THashBucket*) _xgrab(dw * sizeof(THashBucket));
   m_nBuckets     = dw;
   m_nCount       = 0;
   m_pFirstItem   = 0;
   m_dwPopIndex   = 0;
   while( pItem )
   {
      ...
      dw = ( _xstrhash(pItem->m_pKey,pItem->m_cbKey)  &  (m_nBuckets -1) );

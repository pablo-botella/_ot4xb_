...
/*{{|desc: Quick tabbed editor dialog helper, in the style of the old alert() helpers but with edit controls,
      ...
      Xbase++ dialog class.
    | params:
    - `hParent` numeric/object - Parent HWND or Xbase++ window/control object.
    - `aRect` array by reference - Dialog rectangle as {left, top, right, bottom}; updated with the final
      rectangle.
    - `nFlags` numeric - Display and behavior flags combined with OR or nOr().
    - `cTitle` character - Dialog title.
    - `aTabs` array - Tab captions. If omitted, a single tab using the title is created.
    - `aButtons` array - Button captions.
    - `aEdits` array by reference - Edit descriptors. Each item is {nTab, nFlags, cLabel, xValue
      [,`aPickValues | cCaption`]}.
    - `hWndNotify` numeric/object - Optional HWND or Xbase++ window/control object that receives dirty
      dialog notification messages.
    - `nActiveTab` numeric - Initial tab index.
    | flags:
    - `0x00000001` Place on primary monitor.
    - `0x00000002` Place on the monitor that contains hParent.
    - `0x00000003` Place on the monitor that contains the mouse.
    - `0x00000004` Center the dialog.
    - `0x00000008` Maximize inside the selected bounds.
    - `0x00000010` Treat aRect as parent-window coordinates.
    - `0x00000030` Treat aRect as parent-client coordinates.
    - `0x00000100` Hide from taskbar.
    - `0x00000200` Create as child window.
    - `0x00000400` Disable parent while the dialog is active.
    - `0x00000800` Show title/caption behavior.
    - `0x00008000` Run in a new thread and return the thread id.
    - `0x00010000` Top-level mode.
    - `0x00020000` Dummy hit-test behavior for moving/caption handling.
    - `0x00040000` Buttons participate in tab-stop keyboard navigation.
    - `0x00080000` Subclass the parent for dialog coordination.
    - `0x01000000` Add a client edge around the tab client window.

    |note: Flags are combined with OR or nOr(), not with addition. When the dialog runs synchronously, closing
      it with [X] or Alt+F4 returns 0. Pressing a button returns its 1-based position in aButtons and copies
      changed values back into aEdits. With OT4XB_DDF_USETHREAD it starts a thread, returns the thread id and
      does not synchronously collect edited values.

    |example: local rc := {0,0,640,420} local flags := nOr( OT4XB_DDF_CHILD, OT4XB_DDF_COORD_WINDOW,
      OT4XB_DDF_DISPLAY_MAXIMIZED ) local tabs := {"Connection","Options"} local btns := {"OK","Cancel"} local
      edits := { ; {1,0x0800,"Url", cUrl}, ; {1,0x0002,"Retries", cPrintf("%i", nRetries)}, ; {1,0,"HTTP
      method", cMethod, {"GET","POST"}}, ; {2,0,"Use cache", lUseCache, "Cache responses"} ; } local result :=
      ot4xb_dirty_dlgedit( SetAppWindow(), @rc, flags, "Settings", tabs, btns, @edits ) if result == 1 cUrl :=
      edits[1,4] nRetries := Val(edits[2,4]) cMethod := edits[3,4] lUseCache := edits[4,4] endif

    |edit-descriptor: Each aEdits element describes one control as {nTab, nFlags, cLabel, xValue [,xExtra]}.
      ...
      synchronous dialog closes. When xValue is logical, the control is a checkbox and xExtra, when present, is
      the checkbox caption. When xValue is not logical, the value is edited as character data. If xExtra is an
      array, the control is a dropdown combo populated with those pick values; otherwise it is a plain edit
      control.

    |edit-flags: ES_RIGHT: right-align text in the edit control.ES_PASSWORD: password edit control.ES_READONLY:
      read-only edit control; it is not used as a tab-stop target.

    |notifications: Dialog created.Tab control created.Page created.Page container created.Label control
      created.Edit/control created.Button created.Button clicked. }}*/

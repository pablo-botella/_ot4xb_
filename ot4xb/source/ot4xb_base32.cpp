...
namespace base32_ns
{
   ...
   DWORD encode_5_bytes_block( LPSTR pDigitTable, LPBYTE& pSource, DWORD& cbSource, LPSTR& pDestination, DWORD& cbMaxDestination, DWORD& output_bytes_written, DWORD flags )
   {
      ...
      {
         return 0; // Validaci�n de entrada
      }

      BYTE input_block[ 5 ] = { 0 };
      DWORD input_size = ( cbSource < 5 ) ? cbSource : 5; // Tama�o real del bloque de entrada
      ...
      return input_size; // Retornar el tama�o del bloque procesado
   }
...
OT4XB_API BOOL ot4xb_base32_decode_with_table(LPSTR pTable, LPCSTR szEncoded, int nEncodedLen, LPBYTE pDest, int* pnDestLen, DWORD flags) {
    ...
    if (pnDestLen[0] < requiredLength) {
        return FALSE; // Buffer de destino demasiado peque�o
    }
    ...
    if (decodedLength == 0) {
        return FALSE; // Error durante la decodificaci�n
    }

    pnDestLen[0] = (int)decodedLength; // Actualizar el tama�o real de los datos decodificados
    return TRUE; // Decodificaci�n exitosa
}
/*{{end-c-function}}*/